...
export default function BlogListLayout({ blogs, exam }: BlogListLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-surface font-body">
      ...
      {/* Hero */}
      <section className="bg-gradient-to-br from-[#f0f7ff] to-white py-12 md:py-16 px-4 border-b border-accent/20">
        <div className="max-w-[1200px] mx-auto text-center">
          <span className="inline-flex items-center gap-2 bg-secondary/10 text-secondary text-xs font-bold px-3 py-1.5 rounded-full mb-4">
            <span className="material-icons text-sm">auto_stories</span>
            Fryment {exam.name} Blog
          </span>
          <h1 className="text-3xl md:text-5xl font-black text-primary tracking-tight mb-4">
            {exam.name} Preparation, Tips &amp;{' '}
            <span className="text-secondary">Expert Guides</span>
          </h1>
      ...
      {featured && (
        <section className="max-w-[1200px] mx-auto px-4 md:px-6 py-10 md:py-14">
          <Link href={`/blog/${featured.slug}`} className="group rounded-2xl overflow-hidden border border-accent/20 hover:shadow-xl transition-all duration-300 bg-surface">
            <div className="p-8 md:p-10 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-secondary text-white text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full">
                  {featured.category || `${exam.name} Guide`}
                </span>
                <span className="text-xs text-primary/50 font-medium">{featured.read_time || '5 min read'}</span>
              </div>
              <h2 className="text-xl md:text-2xl font-black text-primary leading-snug mb-3 group-hover:text-secondary transition-colors">
                {featured.title}
              </h2>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-xs text-primary/50">
                  <div className="w-7 h-7 rounded-full bg-secondary/10 flex items-center justify-center">
                    <span className="material-icons text-secondary text-sm">person</span>
                  </div>
                  <span className="font-semibold text-primary/80">{featured.author || 'Fryment Team'}</span>
                  <span>•</span>
      ...
      {/* CTA Banner */}
      <section className="bg-primary py-14 px-4 text-center mt-4">
        <div className="max-w-2xl mx-auto">
          ...
          <Link href={`/${exam.id}#purchase`} className="inline-flex items-center gap-2 bg-accent text-primary font-black text-sm px-8 py-3.5 rounded-2xl shadow-lg hover:bg-yellow-300 transition-colors">
