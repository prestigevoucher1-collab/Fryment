...
const FAQS = [
  { q: "How is the TOEFL iBT different from IELTS or TOEFL?", a: "The TOEFL is a fully online, at-home test that takes only 1 hour and costs significantly less than IELTS or TOEFL. Results are available within 48 hours and can be shared with unlimited universities for free. It's ideal for students who want fast, affordable, and convenient certification." },
  { q: "Is the TOEFL iBT accepted in India's top study-abroad destinations?", a: "Yes. Over 4,000 universities in the US, UK, Canada, Australia, Germany, and other countries accept the TOEFL. This includes Ivy League schools, top Canadian colleges, and leading UK institutions." },
  { q: "What is the validity of the TOEFL iBT voucher?", a: "Your Fryment voucher is valid for 21 days from the date of purchase. You must complete your test session within that window. The test result itself is valid for 2 years." },
  ...
  { q: "Can I take the TOEFL iBT from home?", a: "Yes! That's one of the biggest advantages of the TOEFL. You can take it from anywhere using a computer with a webcam and a stable internet connection. No test center required." },
  { q: "What is a good TOEFL iBT score?", a: "Scores range from 10 to 160. Most top universities require 110–120+. An overall score of 120 is considered competitive for admission to the majority of international programs." },
];

const TESTIMONIALS = [
  { name: "Arjun Reddy", city: "Hyderabad", score: "112/120", text: "The support team is amazing. They helped me through the entire TOEFL booking process after I bought the voucher. Saved ₹1400!", rating: 5 },
  { name: "Neha Gupta", city: "Pune", score: "105/120", text: "Got the voucher code instantly and booked my Home Edition test without any credit card issues. Highly recommend Fryment.", rating: 5 },
];

const SCORE_BREAKDOWN = [
  { skill: "Reading", desc: "Understanding academic texts", color: "bg-blue-500", width: "w-[85%]" },
  { skill: "Listening", desc: "Understanding lectures and conversations", color: "bg-teal-500", width: "w-[80%]" },
  { skill: "Speaking", desc: "Expressing an opinion on a familiar topic", color: "bg-violet-500", width: "w-[75%]" },
  { skill: "Writing", desc: "Writing essay responses", color: "bg-amber-500", width: "w-[78%]" }
];

const UNIVERSITIES = [
  "MIT", "Stanford University", "Oxford University", "Cambridge University",
...
export default function TOEFLPage() {
  ...
  const [formData, setFormData] = useState({ fullName: "", phone: "", email: "", state: "", quantity: "1", examType: "TOEFL iBT" });
    ...
    <div className="bg-white min-h-screen text-[#0f172a] font-sans selection:bg-amber-200 overflow-x-hidden">
        ...
        {isBookingModalOpen && (
          ...
          >
            ...
            >
              ...
              {/* Modal Header — TOEFL brand green */}
              <div className="bg-[#2563eb] p-8 text-center relative overflow-hidden">
                ...
                <div className="inline-flex items-center gap-1.5 bg-white/20 text-white px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest mb-3 border border-white/20">
                  <Sparkles className="w-3 h-3 text-[#fde047]" /> Save ₹1,400 Today
                </div>
                <h3 className="text-2xl font-black text-white">Book TOEFL iBT</h3>
              ...
              <div className="p-8">
                {!submitted && (
                  <div className="flex gap-2 mb-8">
                    <div className={cn("h-1.5 rounded-full transition-all duration-500 w-1/2", formStep >= 1 ? "bg-[#2563eb]" : "bg-slate-100")} />
                    <div className={cn("h-1.5 rounded-full transition-all duration-500 w-1/2", formStep >= 2 ? "bg-[#2563eb]" : "bg-slate-100")} />
                  </div>
                )}
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                          <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-3">
                            <div className="flex justify-between text-sm font-medium text-slate-500">
                              <span>Official TOEFL Price</span>
                              <span className="line-through">₹16,900</span>
                            </div>
                            ...
                            <div className="border-t border-slate-200 pt-3 flex justify-between font-black text-lg">
                              ...
                              <span className="text-[#2563eb]">₹15,500</span>
                            </div>
                          </div>
                    ...
                    <div className="pt-2 space-y-3">
                      ...
                      {formStep === 2 && (
                        <button type="button" onClick={() => setFormStep(1)} className="w-full text-center text-xs font-bold text-slate-400 hover:text-[#2563eb] transition-colors">
                          ← Edit Details
                        </button>
                      )}
      ...
      <nav className="fixed top-0 left-0 w-full z-[1000] h-20 bg-white/90 backdrop-blur-xl border-b border-slate-200/50">
        <div className="max-w-7xl mx-auto h-full px-6 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-6">
            ...
            <button onClick={() => setIsBookingModalOpen(true)} className="bg-[#2563eb] text-white font-black rounded-xl px-5 py-2.5 hover:bg-[#1d4ed8] transition-colors shadow-lg shadow-blue-500/20">
              Buy Voucher — ₹15,500
            </button>
          </div>
          {/* Mobile CTA */}
          <button onClick={() => setIsBookingModalOpen(true)} className="lg:hidden bg-[#2563eb] text-white font-black rounded-xl px-4 py-2.5 text-sm hover:bg-[#1d4ed8] transition-colors">
            Buy Now
          </button>
        </div>
      </nav>
      ...
      <main>
        ...
        <section className="relative pt-32 pb-0 px-6 overflow-hidden bg-gradient-to-b from-[#f0f9ff] to-white">
          {/* Decorative blobs */}
          <div className="absolute top-20 left-0 w-72 h-72 bg-[#2563eb]/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <div className="max-w-6xl mx-auto relative z-10">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400 font-medium mb-8">
              ...
              <ChevronRight className="w-4 h-4" />
              <span className="text-[#0f172a] font-bold">TOEFL iBT</span>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_420px] gap-12 items-center pb-16">
              ...
              <div className="space-y-7">
                ...
                <h1 className="text-4xl md:text-5xl lg:text-[3.2rem] font-black text-[#0f172a] leading-[1.1] tracking-tight">
                  Buy TOEFL iBT<br />
                  <span className="text-[#2563eb]">Voucher at ₹15,500</span>
                </h1>

                <p className="text-lg text-slate-500 leading-relaxed max-w-xl">
                  India's most trusted source for discounted TOEFL vouchers. Pay in INR via UPI or Card — zero forex charges, instant WhatsApp delivery.
                </p>

                {/* Quick Stats */}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { icon: Clock, label: "2 Hours", sub: "Test Duration" }, { icon: Zap, label: "6 Days", sub: "Results" },
                    { icon: Laptop, label: "Center/Home", sub: "Test Format" },
                    { icon: Globe, label: "11,500+", sub: "Universities" }
                ...
                <div className="flex flex-wrap gap-4 pt-2">
                  <button onClick={() => setIsBookingModalOpen(true)} className="bg-[#0f172a] text-white font-black text-base rounded-xl px-8 py-4 hover:bg-slate-800 transition-colors shadow-xl flex items-center gap-2">
                    Buy Now — ₹15,500 <ArrowRight className="w-5 h-5" />
                  </button>
              ...
              <div className="bg-white rounded-[28px] p-8 border border-slate-200 shadow-2xl shadow-slate-200/60 relative overflow-hidden">
                <div className="absolute top-0 right-0 bg-[#2563eb] text-white font-black text-xs px-5 py-2 rounded-bl-2xl">Save 14%</div>

                <div className="space-y-6">
                  <div>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Exam</p>
                    <p className="text-xl font-black text-[#0f172a]">TOEFL iBT</p>
                  </div>

                  <div className="bg-[#eff6ff] rounded-2xl p-5 space-y-3">
                    <div className="flex justify-between text-sm font-medium text-slate-500">
                    ...
                    <div className="border-t border-[#2563eb]/20 pt-3 flex justify-between font-black text-2xl">
                      <span>You Pay</span>
                      <span className="text-[#2563eb]">₹15,500</span>
                    </div>
                  </div>
                  ...
                  <button onClick={() => setIsBookingModalOpen(true)} className="w-full bg-[#2563eb] text-white font-black text-lg rounded-xl py-4 hover:bg-[#1d4ed8] transition-colors shadow-lg shadow-blue-400/20 flex items-center justify-center gap-2">
          ...
          <div className="max-w-6xl mx-auto flex flex-wrap justify-center gap-8 md:gap-16">
            {[
              ...
              <div key={text} className="flex items-center gap-2.5 text-white/80 text-sm font-bold">
                <Icon className="w-5 h-5 text-[#2563eb]" /> {text}
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="details" className="py-24 px-6 bg-white">
          <div className="max-w-6xl mx-auto space-y-16">
            <div className="text-center max-w-2xl mx-auto">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">About the Exam</p>
              <h2 className="text-3xl md:text-4xl font-black text-[#0f172a]">What is the TOEFL iBT?</h2>
              <p className="text-slate-500 mt-4 leading-relaxed">The TOEFL is a modern, AI-powered English proficiency test designed for today's global student. It's fast, affordable, and accepted by thousands of universities worldwide.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
              <div className="space-y-6">
                ...
                {[
                  ...
                  { icon: Clock, title: "Under 2 Hours Total", desc: "The new enhanced TOEFL iBT takes less than 2 hours to complete.", color: "text-[#2563eb] bg-[#dbeafe]" },
                  { icon: Laptop, title: "Center or Home", desc: "Choose between taking the test at a secure center or from your own home.", color: "text-violet-600 bg-violet-50" },
                  { icon: Zap, title: "Results in 4-8 Days", desc: "Get your official scores in as little as 4 days.", color: "text-amber-600 bg-amber-50" },
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    { part: "Part 1", title: "Reading (35 min)", desc: "Read passages and answer questions.", badge: "Scored" },
                ...
                <div className="bg-[#eff6ff] rounded-2xl p-6 border border-[#2563eb]/20 mt-6">
                  <h4 className="font-bold text-[#0f172a] mb-4 flex items-center gap-2">
                    <Info className="w-5 h-5 text-[#2563eb]" /> System Requirements
                  </h4>
                  <ul className="space-y-2 text-sm text-slate-600">
                    {[
                      ...
                      <li key={req} className="flex items-center gap-2.5">
                        <Laptop className="w-4 h-4 text-[#2563eb] shrink-0" /> {req}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <section id="score" className="py-24 px-6 bg-[#f8fafc]">
          <div className="max-w-5xl mx-auto space-y-14">
            <div className="text-center">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Understanding Your Score</p>
              <h2 className="text-3xl md:text-4xl font-black text-[#0f172a]">How TOEFL Scores Work</h2>
              <p className="text-slate-500 mt-3 max-w-lg mx-auto">Your TOEFL result includes an overall score and four subscores that measure different language skills.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-start">
              ...
              <div className="space-y-4">
                ...
                {[
                  ...
                  { tier: "Top 50 Global Universities", range: "100 – 120", color: "text-violet-700 bg-violet-50 border-violet-100" },
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 mt-4">
                  <p className="text-sm text-amber-800 font-medium flex items-start gap-2">
                    ...
                    Requirements vary by program and country. Always check the official university admissions page for the exact TOEFL score cutoff.
                  </p>
                </div>
              </div>
            </div>
        ...
        <section className="py-20 px-6 bg-white border-y border-slate-100">
          <div className="max-w-6xl mx-auto space-y-10 text-center">
            <div>
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Global Reach</p>
        ...
        <section className="py-24 px-6 bg-[#f8fafc]">
          <div className="max-w-5xl mx-auto space-y-14">
            <div className="text-center">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Simple Process</p>
              <h2 className="text-3xl md:text-4xl font-black text-[#0f172a]">How to Book Your TOEFL Voucher</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 relative">
              ...
              {[
                ...
                { step: "03", icon: ExternalLink, title: "Go to TOEFL.com", desc: "Visit the official TOEFL iBT website and redeem your code." },
                ...
                <div key={step} className="flex flex-col items-center text-center space-y-4 relative z-10">
                  <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center border border-slate-200 shadow-md text-[#2563eb]">
        ...
        {/* COMPARISON — TOEFL vs Alternatives */}
        <section className="py-24 px-6 bg-white">
          <div className="max-w-5xl mx-auto space-y-12">
            <div className="text-center">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Comparison</p>
              <h2 className="text-3xl md:text-4xl font-black text-[#0f172a]">TOEFL vs. IELTS vs. TOEFL</h2>
              <p className="text-slate-500 mt-3 max-w-lg mx-auto">See why thousands of Indian students are choosing the TOEFL iBT.</p>
            </div>
            ...
            <div className="rounded-3xl overflow-hidden border border-slate-200 shadow-lg">
              <div className="grid grid-cols-4 bg-slate-50 border-b border-slate-200">
                ...
                <div className="p-5 text-center text-sm font-black text-[#2563eb] uppercase tracking-wider border-x border-slate-200 bg-[#eff6ff]">TOEFL</div>
              ...
              {[
                ...
                { feature: "Fryment Price", det: "₹15,500", ielts: "₹17,000", toefl: "₹14,200" },
                { feature: "Test Duration", det: "2 Hours", ielts: "2h 45m", toefl: "2 Hours" },
                { feature: "Result Time", det: "4-8 days", ielts: "13 days", toefl: "48 hours" },
                { feature: "At-Home Test", det: true, ielts: false, toefl: false },
                { feature: "Free Score Sharing", det: false, ielts: false, toefl: false },
                ...
                <div key={feature} className={cn("grid grid-cols-4 border-b border-slate-100 last:border-b-0", i % 2 === 0 ? "bg-white" : "bg-slate-50/50")}>
                  ...
                  <div className="p-5 text-center border-x border-slate-200 bg-[#eff6ff]/50">
                    {det === true ? <CheckCircle2 className="w-5 h-5 text-emerald-500 mx-auto" /> : det === false ? <X className="w-5 h-5 text-slate-300 mx-auto" /> : <span className="text-sm font-black text-[#2563eb]">{det}</span>}
                  </div>
            ...
            <div className="text-center">
              <button onClick={() => setIsBookingModalOpen(true)} className="bg-[#2563eb] text-white font-black rounded-xl px-8 py-4 hover:bg-[#1d4ed8] transition-colors shadow-lg shadow-blue-400/20">
                Get TOEFL Voucher at ₹15,500 →
              </button>
            </div>
          </div>
        </section>
        ...
        <section className="py-24 px-6 bg-[#f8fafc]">
          <div className="max-w-5xl mx-auto space-y-12">
            <div className="text-center">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Student Stories</p>
                ...
                <div key={i} className="bg-white p-7 rounded-3xl border border-slate-200 shadow-sm flex flex-col">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 bg-[#eff6ff] rounded-full flex items-center justify-center font-black text-[#2563eb] text-sm">{r.name.charAt(0)}</div>
                      <div>
        ...
        <section id="faq" className="py-24 px-6 bg-white">
          <div className="max-w-3xl mx-auto space-y-12">
            <div className="text-center">
              <p className="text-xs font-bold text-[#2563eb] uppercase tracking-widest mb-3">Got Questions?</p>
              ...
              {FAQS.map((faq: any, i: number) => (
                <div key={i} className={cn("rounded-2xl border overflow-hidden transition-all", activeFaq === i ? "border-[#2563eb]/30 bg-[#eff6ff]" : "border-slate-100 bg-[#f8fafc]")}>
                  <button onClick={() => setActiveFaq(activeFaq === i ? null : i)} className="w-full flex items-center justify-between p-6 text-left gap-4">
                    ...
                    <div className={cn("shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all", activeFaq === i ? "bg-[#2563eb] text-white rotate-180" : "bg-white border border-slate-200 text-slate-400")}>
        ...
        <section className="py-24 px-6 bg-[#0f172a] text-white relative overflow-hidden">
          <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_30%_50%,_#2563eb,transparent)]" />
          <div className="max-w-4xl mx-auto text-center space-y-8 relative z-10">
            <div className="inline-flex items-center gap-2 bg-white/10 text-white px-4 py-2 rounded-full text-sm font-bold border border-white/10">
              <Flame className="w-4 h-4 text-[#2563eb]" /> Limited time — Save ₹1,400 on your TOEFL
            </div>
            <h2 className="text-4xl md:text-5xl font-black leading-tight">
              Ready to Take the<br />
              <span className="text-[#2563eb]">TOEFL iBT?</span>
            </h2>
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <button onClick={() => setIsBookingModalOpen(true)} className="bg-[#2563eb] text-white font-black text-lg rounded-xl px-10 py-4 hover:bg-[#1d4ed8] transition-colors shadow-xl shadow-blue-500/20 flex items-center gap-2">
                Buy Voucher — ₹15,500 <ArrowRight className="w-5 h-5" />
              </button>
